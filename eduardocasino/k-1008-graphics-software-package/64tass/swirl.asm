...
KIMMON   =      $1C22        ; RESET ENTRY INTO KIM MONITOR
...
SCALE:   LDA    COS          ; X2CORD=NX/2*SIN4NX/2
         ...
         STA    MPCD+1
         LDA    #(NX/2)&$FF  ; TRANSFER NX/2 TO MULTIPLIER
         STA    MPLR         ; (INTEGER)
         LDA    #NX/2/256
         STA    MPLR+1
         ...
         CLC                 ; AND PROD+3 (HIGH)
         ADC    #(NX/2)&$FF  ; ADD NX/2 TO PRODUCT AND PUT INTO X2CORD
         STA    X2CORD
         LDA    PROD+3
         ADC    #NX/2/256
         STA    X2CORD+1
         ...
         STA    MPCD+1
         LDA    #(NY/2)&$FF  ; TRANSFER NY/2 TO MULTIPLIER
         STA    MPLR         ; (INTEGER)
         LDA    #NY/2/256
         STA    MPLR+1
         JSR    SGNMPY       ; PERFORM A SIGNED MULTIPLICATION
         ...
         LDA    PROD+2       ; SIGNED INTEGER RESULT IN PROD+2 (LOW)
         CLC                 ; AND PROD+3 (HIGH)
         ADC    #(NY/2)&$FF  ; ADD NY/2 TO PRODUCT AND PUT INTO Y2CORD
         STA    Y2CORD
         LDA    PROD+3
         ADC    #NY/2/256
         STA    Y2CORD+1
...
RANG3:   LDA    FREQ+1       ; IF FREQ AND DAMP ARE INDIVIDUALLY OK,
         ...
         EOR    $FF          ; ABSOLUTE VALUE OF FREQ IS SMALL
RANG4:   CMP    #8
